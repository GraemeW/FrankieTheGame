...
| :----------------: |:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------:| :---: |:---------------------------------------------------------------------:|
|      **Core**      | Player singleton & state machine, persistent object spawner, game state modifier registration _(backed by com.lowdefmustard.gamestatemodifiers)_, predicate logic/evaluators |       |           [Core](./Core/) <br/> [Predicates](./Predicates/)           |
...
|     **Speech**     |      Dialogue system, including support for complex dialogue trees via Dialogue Nodes & Triggers - interfacing w/ predicate logic _(backed by com.lowdefmustard.utils)_      |       |                          [Speech](./Speech/)                          |
...
| `com.lowdefmustard.gamestatemodifiers`  |              Self-maintaining linkage between `GameStateModifier` assets (game-state-flagged ScriptableObjects) and the in-scene handlers that reference them                |        Cross-cutting — used for any scriptable objects that extend `GameStateModifier`, and with any `IGameStateModifierHandler`s that consume them        |
...
- **`gamestatemodifiers`** — depends on `utils`
