...
- The configuration contract used in `IGameStateModifierHandler` closely mirrors `com.lowdefmustard.localization`'s `ILocalizable` — both are "keep a reverse-index in sync with a MonoBehaviour's lifecycle" contracts solving the same category of problem for different data.
